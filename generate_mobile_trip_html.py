...
<body data-tab="timeline">
  ...
  <script>
    ...
    // 🔍 诊断：测试 autonavi:// 协议瓦片是否可以正常加载
    (function diagTile() {{
      const testUrl = 'autonavi://webrd01.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=7&x=853&y=390&z=10';
      console.log('🗺️ [TileDiag] 正在测试 autonavi:// 伪协议瓦片...');
      fetch(testUrl)
        .then(function(r) {{
          console.log('🗺️ [TileDiag] HTTP状态:', r.status, r.ok ? '✅ 成功' : '❌ 失败');
          return r.blob();
        }})
        .then(function(b) {{ console.log('🗺️ [TileDiag] 瓦片大小:', b.size, 'bytes, type:', b.type); }})
        .catch(function(e) {{ console.error('🗺️ [TileDiag] ❌ 错误:', e.message || e); }});
    }})();
