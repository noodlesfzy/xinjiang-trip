...
<body data-tab="timeline">
  ...
  <script>
    ...
    const mMap = L.map('m-map', {{
      ...
      // 走 127.0.0.1 本地原生流式中继，100% 规避 WebKit 的 Network Offline 拦截
      const localTileUrl = 'http://127.0.0.1:8088/maptile?s={{s}}&style=' + primaryStyle + '&x={{x}}&y={{y}}&z={{z}}';
      const layer = L.tileLayer(localTileUrl, {{
        subdomains: '1234',
        minZoom: 3,
        maxZoom: 18,
